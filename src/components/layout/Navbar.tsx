...
const Navbar: React.FC = () => {
  ...
  // Hide full navbar on landing page and signup page
  const isLandingPage = location.pathname === '/' || location.pathname === '/signup';
  
  // Hide navbar features for non-tourist views (admin, guide, businessman dashboards), login page, and safety page
  const isDashboardPage = location.pathname.includes('/dashboard') || location.pathname.startsWith('/admin') || location.pathname.startsWith('/guide') || location.pathname.startsWith('/businessman');
  const isLoginPage = location.pathname === '/login';
  const isSafetyPage = location.pathname === '/safety';
  const hideNavbarFeatures = isDashboardPage || isLoginPage || isSafetyPage;
  ...
  return (
    <Disclosure as="nav" className={`${isLandingPage ? 'bg-white/10 backdrop-blur-md border-b border-white/20' : 'bg-white shadow-md'} sticky top-0 z-50`}>
      {({ open }) => (
        <>
